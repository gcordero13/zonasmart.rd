...
import { fetchActiveServices, serviceConfig } from '../../services/services'
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <div className="text-center mb-12">
        ...
        <Reveal>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">Nuestros servicios</h1>
          <p className="text-gray-500 max-w-xl mx-auto">
            Instalacion, soporte, limpieza y mantenimiento profesional. Selecciona un servicio y
            cotiza directo con opciones personalizadas.
          </p>
        </Reveal>
      </div>

      {loading && <p className="text-gray-500 text-center py-12">Cargando servicios...</p>}

      {!loading && services.length === 0 && (
        <p className="text-gray-400 text-center py-12">Aun no hay servicios disponibles.</p>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((s, i) => {
          const cfg = serviceConfig(s)
          const hasOpts = cfg.tiers.length > 0 || cfg.items.length > 0
          return (
            <Reveal key={s.id} delay={(i % 3) * 80} variant="reveal-zoom">
              <div
              ...
              >
                  ...
                  <div className="mt-auto">
                    {cfg.tiers.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {cfg.tiers.map((t) => (
                          <span
                            key={t.key}
                            className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-brand/10 text-brand-dark"
                          >
                            {t.label} ${Number(t.price).toFixed(2)}
                          </span>
                        ))}
                      </div>
                    )}
                    {cfg.items.length > 0 && (
                      <p className="text-xs text-gray-400">
                        {cfg.items.length} opcion(es) por cantidad
                      </p>
                    )}
                  </div>
                ) : (
                ...
                <p className="mt-3 text-xs font-semibold text-brand-dark group-hover:underline">
